"""
Script para retreinar o modelo com APENAS as 7 features de input
Sem usar valores 'IGNORADO' - apenas SIM/NÃO
"""
import pandas as pd
...
print("=" * 80)
print("RETREINAMENTO DO MODELO - APENAS 7 FEATURES")
print("=" * 80)
...
# 3. Filtrar: remover apenas valores 'IGNORADO' no alvo, mas manter nas features
print(f"\nAntes da filtragem: {len(df_model)} registros")

# Remover APENAS linhas com 'IGNORADO' no alvo (HOSPITALIZ)
...
print(f"\nDistribuição de valores nas features:")
for col in INPUT_FEATURES:
    if col in df_model.columns:
        print(f"\n{col}:")
        print(df_model[col].value_counts())

# 4. Aplicar One-Hot Encoding nas features categóricas
...
print(f"\nApós one-hot encoding: {df_encoded.shape[1]} colunas")
...
print(f"\nEstatísticas de IDADE (para referência):")
print(f"  Média: {X['IDADE'].mean():.2f}")
print(f"  Desvio padrão: {X['IDADE'].std():.2f}")
print(f"  Mín: {X['IDADE'].min()}")
print(f"  Máx: {X['IDADE'].max()}")

# 8. Escalar IDADE
scaler = StandardScaler()
X['IDADE'] = scaler.fit_transform(X[['IDADE']])

print(f"\nApós escalonamento de IDADE:")
print(f"  Média: {X['IDADE'].mean():.4f}")
print(f"  Desvio padrão: {X['IDADE'].std():.4f}")
